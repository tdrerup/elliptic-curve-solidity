...
class Recipe {
    ...
    execute(options) {
        return __asyncGenerator(this, arguments, function* execute_1() {
            ...
            yield __await(yield* __asyncDelegator(__asyncValues(update({ message: "Preserving to IPFS..." }))));
            const ipfs = yield __await(yield* __asyncDelegator(__asyncValues(connect_1.connect({ address, controls }))));
            const { source } = Preserve.Targets.normalize(inputs["fs-target"]);
            const { cid } = yield __await(yield* __asyncDelegator(__asyncValues(upload_1.upload({ source, ipfs, controls }))));
