...
function clear(options) {
    return __asyncGenerator(this, arguments, function* clear_1() {
        ...
        const task = yield __await(yield* __asyncDelegator(__asyncValues(controls.step({
            message: "Clearing existing bucket contents..."
        }))));
        const { item: { items } } = yield __await(buckets.listPath(bucketKey, "."));
        try {
            for (const item of items) {
                ...
                yield __await(buckets.removePath(bucketKey, item.name));
            }
